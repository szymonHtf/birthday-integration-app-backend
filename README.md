...
```bash
birthday-integration-app-backend$ sam build --use-container
```
...
```bash
birthday-integration-app-backend$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
birthday-integration-app-backend$ sam local start-api
birthday-integration-app-backend$ curl http://localhost:3000/
```
...
```bash
birthday-integration-app-backend$ sam logs -n HelloWorldFunction --stack-name birthday-integration-app-backend --tail
```
...
```bash
birthday-integration-app-backend$ pip install -r tests/requirements.txt --user
# unit test
birthday-integration-app-backend$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
birthday-integration-app-backend$ AWS_SAM_STACK_NAME="birthday-integration-app-backend" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "birthday-integration-app-backend"
```
